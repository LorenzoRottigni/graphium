...
graph! {
    ...
    CreateProductGraph(name: String, price: String) -> (product_dto: Json<Product>) {
...
graph! {
    #[metadata(context = Context, async = true)]
    GetProductGraph(product_id: i64) -> (product_dto: Json<Product>) {
...
graph! {
    #[metadata(context = Context, async = true)]
    ListProductsGraph(limit: i64, offset: i64) -> (products_dto: Json<Vec<Product>>) {
...
graph! {
    #[metadata(context = Context, async = true)]
    UpdateProductGraph(product_id: i64, update: UpdateProduct) -> (product_dto: Json<Product>) {
...
graph! {
    #[metadata(context = Context, async = true)]
    DeleteProductGraph(product_id: i64) -> (result_dto: Json<DeleteResult>) {
