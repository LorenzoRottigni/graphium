...
fn synthesize_registry_bits(
    ...
    if !item_fn.sig.inputs.is_empty() {
        panic!("graphium test functions with `#[for_node(...)]` cannot take parameters");
    }
    ...
    let runtime_fn_ident = format_ident!("__graphium_runtime_node_case_{}", fn_ident);
    let runner_ident = format_ident!("__graphium_runtime_node_runner_{}", fn_ident);
